...
-- а не в клиенте.
--
-- Одна заявка на НАПРАВЛЕНИЕ за всё время, в любом статусе — не одна на пару.
-- Отказ окончателен для отказавшего направления, и это осознанно: возможность
-- просить снова превращает отказ в переговоры. Передумавший при этом
-- отправляет СВОЮ заявку в обратную сторону, и она законна — это уже его
-- решение, а не повторная просьба.
--
-- Шапка говорила «одна на пару» и ссылалась на «второй, симметричный индекс
-- ниже». Такого индекса тут нет и не должно быть: он запретил бы ровно тот
-- обратный ход, который абзацем выше назван законным. Формулировка
-- исправлена задним числом (20260905100000, тринадцатый аудит) — правится
-- ТОЛЬКО текст комментария, ни одной строки DDL миграция не меняла и не
-- меняет; в схеме всегда стоял направленный индекс.
-- =====================================================================
...
-- Уникальность поэтому по (requester, recipient) — и только по ней: это и
-- есть «одна заявка на направление», см. шапку. Второго, симметричного
-- индекса нет намеренно; два индекса ниже — обычные lookup'ы под списки
-- входящих и исходящих, уникальности они не держат.
