...
4. ⏳ Тест вручную на двух аккаунтах — **ещё не выполнен**, эмулятор Android пока не создан (только Android Studio без AVD). Сделать перед стартом Этапа 3: создать AVD через Android Studio Device Manager (или взять физическое Android-устройство), пройти полный цикл — регистрация двух пользователей → создание кода у одного → активация у другого → проверить, что `connections`-запись появилась и видна обоим.
