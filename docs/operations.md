...
2. Рассылка «обновитесь», если она нужна:
   ...
   правда: без обновления старый клиент показывает не то. Так было с 0.17.0 —
   build 43 и старше не фильтрует `posts.in_general_feed` и показывает пост,
   адресованный только в комнату, ещё и в общей ленте. Если размечать важным
   каждый релиз, к третьему разу слово перестанет что-либо значить, а обычный
