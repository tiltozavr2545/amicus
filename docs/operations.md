...
предыдущая версия. Так, deep link по тапу (`data` в сообщении FCM) не
заработает от одного релиза приложения: `data` кладёт функция, и без её
выкладки тап по-прежнему просто открывает приложение. Новый вид уведомления — это всегда ДВЕ правки: `kind` в
