...
import { ago, date, day, Fail } from '../components/ui';
...
export function Users() {
  ...
  const rows = useMemo(() => {
    ...
    const filtered = needle
      ? data.users.filter((u) =>
          [u.name, u.email ?? '', u.id].some((field) =>
          ...
          <tbody>
            {rows.map((u) => (
              <tr key={u.id}>
                ...
                <td className={`nowrap ${u.lastActiveAt ? '' : 'muted'}`}>
                  {ago(u.lastActiveAt)}
                </td>
                <td className="mono">
                  {u.versions.length ? (
                  ...
                  {u.maxBuild !== null ? (
                    <span className="muted"> +{u.maxBuild}</span>
                  ) : null}
                  {u.devices > 1 ? (
                    <span className="muted"> ({u.devices} устр.)</span>
                  ) : null}
                </td>
